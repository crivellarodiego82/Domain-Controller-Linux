...
if [[ 130 -gt $DC_STEP ]] || [ -z ${DC_STEP+x} ] ; then
...
cp "$0" /etc/dc/dc_install.sh
chmod +x /etc/dc/dc_install.sh
#touch /etc/dc/dc_install_variables.sh
#echo '#!/bin/bash' >> /etc/dc/dc_install_variables.sh
#chmod +x /etc/dc/dc_install_variables.sh
fi
...
FQDN=SERVER.local #This is your domain name.
#EXTENSION=.local
NBIOS=SERVER #This is the second level domain name capitalized.
DCNAME=DC
...
ADMINPWD=‘P@ssw0rd’ #This is the password of the Active Directory "Administrator"-account.
...
DHCP_SERVER=0 #Enable this only if you know that this is needed.
	DHCP_SUBNET=192.168.1.1
	DHCP_SUBNETMASK=255.255.255.0
	DHCP_BROADCASTADDRESS=192.168.1.255
	...
	DHCP_FIRST_IP_ADDRESS=192.168.1.10
	DHCP_LAST_IP_ADDRESS=192.168.1.240
...
OR_PATH_FOLDER_SELINUX_POLICY='/etc/selinux/dc_rules'
...
LOCATION_OF_IMPORTANT_FILES=/etc/dc
fi

# Not Needed anymore! This was written when step where being stored in seperate files. #To provide step control. Check if the DC_STEP variabel exists, and if delete it.
#clear_exported_DC_STEP(){
#	sed -i '/DC_STEP/d' /etc/dc/dc_install_variables.sh
#}
...
#cp "$(readlink -f $0" "$LOCATION_OF_IMPORTANT_FILES/Tools/dc_install.sh"

#Check if the command afterwards has allready been added.
#sed -i '/dc_install_variables.sh/d' /root/.bashrc
#Run this script, to add the permanently stored variables, whenever the root user logs in. 
#echo '/etc/dc/dc_install_variables.sh' >> /root/.bashrc

#Check if the command afterwards has allready been added.
sed -i '/dc_install.sh/d' /root/.bashrc
#Run the script whenever the root user logs in.
echo '/etc/dc/dc_install.sh' >> /root/.bashrc

DC_STEP=140; sed -i '2s/DC_STEP=.*/DC_STEP=140/' /etc/dc/dc_install.sh
fi
...
#	echo "2. Files will be stored on removable USB storage: $(if [ "$PORTABLE_CONFIGURATION" -eq "1" ]; then echo "Yes"; else echo "No"; fi)";
	...
	echo "5. Administrator Password: $(if [ "$ADMINPWD" = ‘P@ssw0rd’ ]; then echo ‘P@ssw0rd’; else echo "Hidden! "; fi)";
	...
	echo "SSH Password: $(if [ "$SSH_USER_PASSWORD" = ‘P@ssw0rd’ ]; then echo ‘P@ssw0rd’; else echo "Hidden! "; fi)";
...
DC_STEP=220; sed -i '2s/DC_STEP=.*/DC_STEP=220/' /etc/dc/dc_install.sh
fi
...
DC_STEP=230; sed -i '2s/DC_STEP=.*/DC_STEP=230/' /etc/dc/dc_install.sh
reboot
fi
...
# Generated by the Domain Controller install script $DC_SCRIPT_VERSION
# Generated on $(date)
#
...
DC_STEP=240; sed -i '2s/DC_STEP=.*/DC_STEP=240/' /etc/dc/dc_install.sh
fi
...
DC_STEP=310; sed -i '2s/DC_STEP=.*/DC_STEP=310/' /etc/dc/dc_install.sh
fi
...
if [ "$SELINUX" -eq "1" ]; then
	...
	DC_STEP=315; sed -i '2s/DC_STEP=.*/DC_STEP=315/' /etc/dc/dc_install.sh
	reboot
	fi
...
EOT
	...
	DC_STEP=320; sed -i '2s/DC_STEP=.*/DC_STEP=320/' /etc/dc/dc_install.sh
fi
...
DC_STEP=330; sed -i '2s/DC_STEP=.*/DC_STEP=330/' /etc/dc/dc_install.sh
fi
...
# 3.3.                Configure the timezone              #
###########################################################
if [[ $DC_STEP -eq 330 ]] ; then

timedatectl set-timezone $REGION/$TIMEZONE
timedatectl

DC_STEP=340; sed -i '2s/DC_STEP=.*/DC_STEP=340/' /etc/dc/dc_install.sh
fi

###########################################################
# 3.4.            Configure the hosts file                #
###########################################################
if [[ $DC_STEP -eq 340 ]] ; then
...
DC_STEP=350; sed -i '2s/DC_STEP=.*/DC_STEP=350/' /etc/dc/dc_install.sh
fi
...
DC_STEP=360; sed -i '2s/DC_STEP=.*/DC_STEP=360/' /etc/dc/dc_install.sh
fi
...
DC_STEP=370; sed -i '2s/DC_STEP=.*/DC_STEP=370/' /etc/dc/dc_install.sh
fi
...
DC_STEP=380; sed -i '2s/DC_STEP=.*/DC_STEP=380/' /etc/dc/dc_install.sh
fi
...
DC_STEP=410; sed -i '2s/DC_STEP=.*/DC_STEP=410/' /etc/dc/dc_install.sh
fi
...
cat <<EOT >> "/etc/dc/DC_generate_2fa.sh"
pause_with_msg(){
	read -r -p "Press [Enter] to continue..." fackEnterKey
}
...
chmod +x /etc/dc/DC_generate_2fa.sh
sed -i '//etc/dc/DC_generate_2fa.sh/d' "/$SSH_USER/.bashrc"
...
DC_STEP=420; sed -i '2s/DC_STEP=.*/DC_STEP=420/' /etc/dc/dc_install.sh
fi
...
DC_STEP=430; sed -i '2s/DC_STEP=.*/DC_STEP=430/' "/etc/dc/dc_install.sh"
fi
...
if [ "$PXE_SERVER" -eq "1" ] ; then
	...
	mkdir /dc_temp/dc -f
	curl http://git.ipxe.org/releases/wimboot/wimboot-latest.tar.gz /dc_temp/dc/wimboot-latest.tar.gz
	tar xvf wimboot-latest.tar.gz -C /dc_temp/dc/wimboot
	find /dc_temp/dc/wimboot -name 'wimboot' -exec cp {} "$PXE_HTTP_ROOT/"  \;
	rm -Rf /dc_temp
...
DC_STEP=440; sed -i '2s/DC_STEP=.*/DC_STEP=440/' "/etc/dc/dc_install.sh"
fi
...
DC_STEP=450; sed -i '2s/DC_STEP=.*/DC_STEP=450/' "/etc/dc/dc_install.sh"
fi

###########################################################
# 4.5.  (Optional) Configure automatic security updates   #
###########################################################
if [[ $DC_STEP -eq 450 ]] ; then
...
DC_STEP=460; sed -i '2s/DC_STEP=.*/DC_STEP=460/' "/etc/dc/dc_install.sh"
fi

###########################################################
# 4.6.      (Optional) Test & Enable the Firewall         #
###########################################################
if [[ $DC_STEP -eq 460 ]] ; then
...
DC_STEP=470; sed -i '2s/DC_STEP=.*/DC_STEP=470/' "/etc/dc/dc_install.sh"
fi
...
DC_STEP=480; sed -i '2s/DC_STEP=.*/DC_STEP=480/' "/etc/dc/dc_install.sh"
...
DC_STEP=510; sed -i '2s/DC_STEP=.*/DC_STEP=510/' "/etc/dc/dc_install.sh"
fi
...
DC_STEP=520; sed -i '2s/DC_STEP=.*/DC_STEP=520/' "/etc/dc/dc_install.sh"
fi
...
DC_STEP=530; sed -i '2s/DC_STEP=.*/DC_STEP=530/' /etc/dc/dc_install.sh
fi

###########################################################
# 5.3.              Generate the modulesfile              #
###########################################################
if [[ $DC_STEP -eq 530 ]] ; then
...
DC_STEP=540; sed -i '2s/DC_STEP=.*/DC_STEP=540/' "/etc/dc/dc_install.sh"
fi
...
cat <<'EOT' >> "/etc/dc/DC_locationscript.sh"
	read -r $LAST_LOIF < "/etc/dc/DC_loip"
...
chmod 744 "/etc/dc/DC_locationscript.sh"

touch "/lib/systemd/system/DC_LocationScript.service"
cat <<'EOT' >> "/lib/systemd/system/DC_LocationScript.service"
[Unit]
Description=DC LocationScript Service
After=multi-user.target

[Service]
Type=idle
ExecStart=/etc/dc/DC_locationscript.sh

[Install]
WantedBy=multi-user.target
EOT

chmod 644 "/lib/systemd/system/DC_LocationScript.service"
systemctl daemon-reload
systemctl enable "DC_LocationScript.service"
